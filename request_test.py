...
# print(r.status_code)
# print(len(r.text))
# formData = {'namefield': 'huangjun', 'pwdfield': 'huang123'}
# headers = {
#     'User-Agent':
#     'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:9.0.1) Gecko/20100101 Firefox/52.0'
# }
# res = requests.get(url, data=formData, headers=headers)
...
mafengwoSession.cookies = cookielib.LWPCookieJar(
    filename="mafengwoCookies.txt")

userAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36"
header = {
    # "origin": "http://byxoa.byx-its.com:88/byxoa",
...
def mafengwoLogin(account, password):
    print("开始模拟登录马蜂窝")
    postUrl = "http://byxoa.byx-its.com:88/byxoa/login.do"
    postData = {
        "passport": account,
        "password": password,
    }
...
def isLoginStatus():
    ...
    responseRes = mafengwoSession.get(routeUrl,
                                      headers=header,
...
if __name__ == "__main__":
    ...
    resp = mafengwoSession.get("http://www.mafengwo.cn/plan/fav_type.php",
                               headers=header,
                               allow_redirects=False)
