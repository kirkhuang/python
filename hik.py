...
class LPNET_DVR_DEVICEINFO_V30(ctypes.Structure):
    _fields_ = [
        ...
        ("byAlarmOutPortNum", ctypes.c_byte), ("byDiskNum", ctypes.c_byte),
        ("byDVRType", ctypes.c_byte), ("byChanNum", ctypes.c_byte),
        ("byStartChan", ctypes.c_byte), ("byAudioChanNum", ctypes.c_byte),
        ("byIPChanNum", ctypes.c_byte), ("byZeroChanNum", ctypes.c_byte),
        ("byMainProto", ctypes.c_byte), ("bySubProto", ctypes.c_byte),
        ("bySupport", ctypes.c_byte), ("bySupport1", ctypes.c_byte),
        ("bySupport2", ctypes.c_byte), ("wDevType", ctypes.c_uint16),
        ("bySupport3", ctypes.c_byte), ("byMultiStreamProto", ctypes.c_byte),
        ("byStartDChan", ctypes.c_byte), ("byStartDTalkChan", ctypes.c_byte),
        ("byHighDChanNum", ctypes.c_byte), ("bySupport4", ctypes.c_byte),
        ("byLanguageType", ctypes.c_byte), ("byVoiceInChanNum", ctypes.c_byte),
        ("byStartVoiceInChanNo", ctypes.c_byte), ("byRes3", ctypes.c_byte * 2),
...
def NET_DVR_Login_V30(sDVRIP="32.91.106.12",
                      wDVRPort=8000,
                      sUserName="admin",
                      sPassword="12345"):
    ...
    print(DeviceInfo)
    lUserID = callCpp("NET_DVR_Login_V30", sDVRIP, wDVRPort, sUserName,
                      sPassword, ctypes.byref(DeviceInfo))
...
class NET_DVR_PREVIEWINFO(ctypes.Structure):
    _fields_ = [("lChannel", ctypes.c_long), ("lLinkMode", ctypes.c_long),
                ("hPlayWnd", ctypes.c_void_p),
                ("sMultiCastIP", ctypes.c_char_p),
                ("byProtoType", ctypes.c_byte), ("byRes", ctypes.c_byte * 3)]
...
def Preview():
    ...
    m_lRealHandle = callCpp("NET_DVR_RealPlay_V30", lUserID,
                            ctypes.byref(lpPreviewInfo), None, None, True)
    if (m_lRealHandle < 0):
        error_info = callCpp("NET_DVR_GetLastError")
...
class NET_DVR_JPEGPARA(ctypes.Structure):
    _fields_ = [("wPicSize", ctypes.c_ushort),
...
def Get_JPEGpicture():
    ...
    if (callCpp("NET_DVR_CaptureJPEGPicture", lUserID, lChannel,
                ctypes.byref(lpJpegPara), sJpegPicFileName) == False):
        error_info = callCpp("NET_DVR_GetLastError")
        print("抓图失败：" + str(error_info))
    else:
        print("抓图成功")
...
#     _fields_ = [("dwSize", ctypes.c_uint32), ("byFocusMode", ctypes.c_byte),
...
#     if (callCpp("NET_DVR_GetDVRConfig", lUserID, 3305, lChannel,
#                 ctypes.byref(m_struFocusModeCfg), 76,
...
#         if (callCpp("NET_DVR_SetDVRConfig", lUserID, 3306, lChannel,
#                     ctypes.byref(m_struFocusModeCfg), 76) == False):
#             error_info = callCpp("NET_DVR_GetLastError")
#             print("光学变倍修改失败：" + str(error_info))
#         else:
#             print("光学变倍修改成功;修改后的数据为：" +
#                   str(m_struFocusModeCfg.fOpticalZoomLevel))
...
#     _fields_ = [("dwSize", ctypes.c_uint32), ("lpOutBuffer", ctypes.c_void_p),
...
#     inPutBuffer = "<" + choise + "><StartPoint><positionX>" + str(
#         startX) + "</positionX><positionY>" + str(
#             startY) + "</positionY></StartPoint><EndPoint><positionX>" + str(
#                 endX) + "</positionX><positionY>" + str(
...
#     if (callCpp("NET_DVR_STDXMLConfig", lUserID, ctypes.byref(struInput),
#                 ctypes.byref(struOuput))):
#         error_info = callCpp("NET_DVR_GetLastError")
...
#     _fields_ = [("dwSize", ctypes.c_uint32), ("sPassword", ctypes.c_byte * 16),
...
#     if (callCpp("NET_DVR_ActivateDevice", DevAddr, 8000,
#                 ctypes.byref(struActivateCfg))):
#         error_info = callCpp("NET_DVR_GetLastError")
...
# #区域局部聚焦和局部曝光功能：矩形区域坐标左上角和右下角（startX,startY,endX,endY）
# # flag=1局部聚焦功能，flag!=1局部曝光功能
# # https://blog.csdn.net/jacke121/article/details/89327466